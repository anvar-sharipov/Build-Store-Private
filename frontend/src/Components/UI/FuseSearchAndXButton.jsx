...
const FuseSearchAndXButton = ({ fetchedLists, labelName, object, setObject, setFilteredList, placeHolderText, filteredList }) => {
  const { t } = useTranslation();
  
  const fuse = useMemo(
    () =>
      new Fuse(fetchedLists, {
        keys: ["name"],
        threshold: 0.3,
      }),
    [fetchedLists]
        ...
        >
          <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border-2 border-cyan-500/30 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-400 mb-0.5">{t("choosed")}</p>
...
export default FuseSearchAndXButton;