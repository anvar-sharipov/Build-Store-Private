...
const TFoot = ({ printVisibleColumns, visibleColumns }) => {
  ...
  const total_purchase = values.products.reduce((sum, p) => {
    return sum + (Number(p.purchase_price) || 0) * (Number(p.selected_quantity) || 0);
  }, 0);


  const total_income = values.products.reduce((sum, p) => {
    const income = (Number(p.selected_price) - Number(p.purchase_price)) * Number(p.selected_quantity);
    return sum + income;
  }, 0);

  const total_selected_price = values.products.reduce((sum, p) => {
    return sum + Number(p.selected_price || 0) * Number(p.selected_quantity || 0);
  }, 0);

  const total_discount_price = values.products.reduce((sum, p) => {
  ...
  return (
    <tfoot>
      <tr className="dark:bg-gray-900 dark:text-gray-300 bg-gray-200">
        ...
        >
          {formatNumber(total_discount_price, 3)}
        </th>
        <th
