...
const exportInvoiceWithSaldoToExcel = async (values, visibleColumns, printVisibleColumns, t, awtoData, saldoForExcel, saldo2) => {
  ...
  try {
    ...
    values.products.forEach((product, index) => {
      ...
      columnMapping.forEach((col) => {
        ...
        switch (col.key) {
          ...
          case "price":
            // Цена: всегда 3 знака после запятой
    ...
    worksheet.getColumn(3).width = 15; // Дебет
    worksheet.getColumn(4).width = 15; // Кредит
    ...
    if (saldo && (saldo["60_USD"] || saldo["75_USD"])) {
      const addCompactAccountTable = (accountKey, accountName) => {
        ...
        totalRow.getCell(3).value = safeNumber(accountData.final?.[0], 2);
        ...
        totalRow.getCell(4).value = safeNumber(accountData.final?.[1], 2);
...
//         openingRow.getCell(3).value = safeNumber(start_debit);
//         openingRow.getCell(3).numFmt = "#,##0.000";
//         openingRow.getCell(3).style = { ...styles.saldoData, alignment: { horizontal: "right" } };
//         // openingRow.getCell(4).value = formatNumber2(start_credit) || "0";
//         openingRow.getCell(4).value = safeNumber(start_credit);
//         openingRow.getCell(4).numFmt = "#,##0.000";
//         openingRow.getCell(4).style = { ...styles.saldoData, alignment: { horizontal: "right" } };
//         applySaldoRowBorder(openingRow);
//         currentRow++;
...
//             const debitCell = dataRow.getCell(3);
//             debitCell.value = safeNumber(entry[2]); // ТОЛЬКО number | null
//             debitCell.numFmt = "#,##0.000";
...
//         totalRow.getCell(3).value = safeNumber(accountData.final?.[0]);
//         totalRow.getCell(3).numFmt = "#,##0.000";
//         totalRow.getCell(3).style = { ...styles.saldoData, alignment: { horizontal: "right" }, font: { bold: true } };
//         // totalRow.getCell(4).value = formatNumber2(accountData.final?.[1]) || "0";
//         totalRow.getCell(4).value = safeNumber(accountData.final?.[1]);
...
//         closingRow.getCell(3).value = safeNumber(accountData.saldo?.[0]);
//         closingRow.getCell(3).numFmt = "#,##0.000";
//         closingRow.getCell(3).style = { ...styles.saldoData, alignment: { horizontal: "right" }, font: { bold: true } };
//         // closingRow.getCell(4).value = accountData.saldo?.[1] !== "0" ? formatNumber2(accountData.saldo?.[1]) : "-";
//         closingRow.getCell(4).value = safeNumber(accountData.saldo?.[1]);
//         closingRow.getCell(4).numFmt = "#,##0.000";
//         closingRow.getCell(4).style = { ...styles.saldoData, alignment: { horizontal: "right" }, font: { bold: true } };
//         applySaldoRowBorder(closingRow, true);
//         currentRow += 2;
//       };
