...
const TDGifts = ({ visibleColumns, printVisibleColumns, handleDeleteGift }) => {
      ...
      {values.gifts.map((product, index) => {
        ...
        return (
          <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors bg-green-50 dark:bg-green-900/20">
            ...
            <td className="px-3 py-2 font-semibold text-gray-700 dark:text-gray-300 text-left border border-gray-300 dark:border-gray-600 print:text-[14px] print:!text-black print:px-1 print:py-0.5">{product.name}</td>

            <td className={`px-3 py-2 border border-gray-300 dark:border-gray-600 print:px-1 print:py-0.5 print:!text-black text-center ${visibleColumns.image ? "table-cell" : "hidden"} ${printVisibleColumns.image ? "print:table-cell" : "print:hidden"}`}>
              {product.images.length > 0 && <img src={`${BASE_URL}${product.images[0].image}`} alt={product.images[0].alt_text || "Product image"} className="w-24 h-24 object-cover rounded inline-block" />}
            </td>

            <td className={`px-3 py-2 border border-gray-300 dark:border-gray-600 print:px-1 print:py-0.5 print:!text-black text-center ${visibleColumns.qr_code ? "table-cell" : "hidden"} ${printVisibleColumns.qr_code ? "print:table-cell" : "print:hidden"}`}>
            ...
            <td className="px-3 py-2 text-gray-900 dark:text-gray-100 text-left border border-gray-300 dark:border-gray-600 print:px-1 print:py-0.5 print:!text-black">{product.unit_name_on_selected_warehouses}</td>
